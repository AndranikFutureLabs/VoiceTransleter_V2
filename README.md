...
- Исправлена проблема с двойной сборкой релиза (тег пересоздан на чистом коммите)
...
- XTTS v2 не требует `monotonic_align` (нужно только для VITS/Glow-TTS)
...
- **DISTUTILS_USE_SDK=1** — обходит проверку MSVC
- Progress-лог MinGW каждые 10% (без спама)

## Что нового в версии 2.2.6

- **Автоматическая установка MinGW-w64** (C-компилятор gcc) для сборки TTS — приложение само скачивает winlibs (260 МБ)
- **Переменные CC/CXX** — pip точно находит gcc.exe/g++.exe
- **Progress-бар** при скачивании MinGW

## Что нового в версии 2.2.5

- **Автоматическая установка MinGW-w64** — первая попытка (URL исправлен в 2.2.6)
- **Исправлена установка TTS** — обновление pip/setuptools/wheel, `--no-build-isolation` с самого начала

## Что нового в версии 2.2.4

- **Исправлена установка TTS** — предустановка numpy/cython, фикс `transformers<4.44`
- **Модели не загружаются заново** (2.2.2) — при перезапуске проверяется кэш, автозагрузка только если кэша нет
- **Реальная проверка кэша TTS** (2.2.2) — проверка `model.pth` вместо заглушки `return true`
- **FFmpeg встроен в Windows-сборку** (2.2.1) — CI скачивает FFmpeg binaries при сборке

## Что нового в версии 2.2.0

- **Исправлен таймаут распознавания речи** — увеличен с 10 до 30 минут (для длинных видео на CPU)
- **Ускорено распознавание** — `beam_size` уменьшен с 5 до 1, включён VAD-фильтр (пропуск тишины), ~3-5× ускорение на CPU
- **Progress-логи при распознавании** — каждые 20 сегментов выводится прогресс, видно что процесс не завис
- **Очистка таймера** — таймаут-таймер очищается при завершении, не висит в памяти
...
1. Скачайте последний релиз: `VoiceTransleter V2 Setup 2.4.0.exe`
